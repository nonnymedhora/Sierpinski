...
juliaColours[i]=colours[i];renderJulia();}}
function toggleJulia()
{if(drawJuliaBox.checked==true)
drawJulia=1;else
drawJulia=0;if(drawJulia){juliaArea.style.display="block";juliaParamsArea.style.display="block";juliaSwitchArea.style.background="#efc";juliaSwitchArea.style.border="1px solid black";juliaIterations=Math.floor(juliaIterSlider.value);updateCoords(canvasWidth/2,canvasHeight/2,"centre");}
...
step=0.05;for(i=-6;i<=6;i+=step){if(Math.abs(i)>0.01){if(i%1.0==0)
tickSize=4;else
...
{julia=new Uint8Array(e.data.julia);renderJulia();}
function renderJulia()
{if(!drawJulia)
return 0;var pixels=mjuliaOffScreenCtx.data;for(let y=0;y<juliaOffScreen.height;y++){var yOffset=y*juliaOffScreen.width;for(let x=0;x<juliaOffScreen.width;x++){out=julia[x+y*juliaOffScreen.width];if(out==255){r=0;g=0;b=0;}else{packedColour=juliaColours[out];r=(packedColour>>24)&0xff;g=(packedColour>>16)&0xff;b=(packedColour>>8)&0xff;}
pixelPos=(x+yOffset)<<2;pixels[pixelPos]=r;pixels[++pixelPos]=g;pixels[++pixelPos]=b;pixels[++pixelPos]=255;}}
juliaOffScreenCtx.putImageData(mjuliaOffScreenCtx,0,0);jctx.drawImage(juliaOffScreen,0,0,juliaCanvasWidth/2,juliaCanvasHeight/2);if(showAxes){var originX=Math.round(juliaCanvasWidth/4);var originY=Math.round(juliaCanvasHeight/4);jctx.translate(0.5,0.5);jctx.strokeStyle="#DDDDDD";jctx.font="10px Sans-serif";jctx.lineWidth=1.0;jctx.beginPath();jctx.moveTo(0,originY);jctx.lineTo(juliaCanvasWidth/2,originY);jctx.stroke();jctx.beginPath();jctx.moveTo(originX,0);jctx.lineTo(originX,juliaCanvasHeight/2);jctx.stroke();jctx.beginPath();jctx.arc(originX,originY,5,0,2*Math.PI);jctx.stroke();var step=0.5;for(i=-3;i<=3;i+=step){if(i!=0){if(i%1.0==0)
tickSize=4;else
...
pixelPos=(x+yOffset)<<2;pixels[pixelPos]=r;pixels[++pixelPos]=g;pixels[++pixelPos]=b;pixels[++pixelPos]=255;}}
lstartLine=Math.floor(workerID*(ultraHeight/workers));ultraCanvasCtx.putImageData(multraSegment,0,lstartLine);workersRunning--;ultraSegment=null;if(workersRunning==0){ultraScaledCanvasCtx.drawImage(ultraCanvas,0,0,ultraWidth/2,ultraHeight/2);document.getElementById("poster").value="Poster";document.getElementById("save").disabled=false;document.getElementById("cycle").disabled=false;nextPalette.disabled=false;prevPalette.disabled=false;iterSlider.disabled=false;itersInput.disabled=false;juliaIterSlider.disabled=false;showAxesBox.disabled=false;drawJuliaBox.disabled=false;lockJuliaColoursBox.disabled=false;smoothBox.disabled=false;jumpSelect.disabled=false;needRedraw=0;var now=new Date();var timeStamp=""+now.getFullYear()+pad(now.getMonth()+1)+pad(now.getDate())+"-"+pad(now.getHours())+pad(now.getMinutes())+pad(now.getSeconds());dLink=document.createElement('a');dLink.setAttribute('download',"mandel-"+timeStamp+".png");dLink.innerHTML="Poster created "+timeStamp+"<br><br>";var thePNG=ultraScaledCanvas.toDataURL("image/png");var byteString=atob(thePNG.split(',')[1]);var mimeString=thePNG.split(',')[0].split(':')[1].split(';')[0];var ab=new ArrayBuffer(byteString.length);var ia=new Uint8Array(ab);for(var i=0;i<byteString.length;i++){ia[i]=byteString.charCodeAt(i);}
...
{if(travelling>0)
return;rotating=1-rotating;if(rotating){cycleText.value="Stop";}else{cycleText.value="Cycle";startRender(0,0);}}
...
else{ldestX=destX;ldestY=destY;ldestZoom=destZoom;if((xnorm!=ldestX)||(ynorm!=ldestY)||(zoom!=ldestZoom)||(iterations<destIters)){if(iterations<destIters){iterations+=benchmarkTime*2;if(iterations>destIters)
...
{var xnorm=Math.floor((x-screenX)/zoom*1000000000)/1000000000;var ynorm=Math.floor((y-screenY)/zoom*1000000000)/1000000000;xPosText.textContent=xnorm;yPosText.textContent=ynorm;coordSourceText.textContent=" "+source;coordSource2Text.textContent=" "+source;oneShotWorker.postMessage({oneShot:1,x:x,y:y,screenX:screenX,screenY:screenY,zoom:zoom,iterations:iterations,smooth:0});if((drawJulia)&&(!drawingJulia)){drawingJulia=1;if(!juliaWorker){juliaWorker=new Worker("julia-compute.js");juliaWorker.onmessage=onJuliaComputeEnded;}
...
function setup()
{setViewport();showAxesBox.checked=false;drawJuliaBox.checked=false;lockJuliaColoursBox.checked=false;smoothBox.checked=false;juliaArea.style.display="none";juliaParamsArea.style.display="none";incrPalette();needRedraw=0;loadState();zoomText.textContent=Math.floor(zoom);oneShotWorker=new Worker("mandel-compute.js");oneShotWorker.onmessage=onOneShotComputeEnded;for(let i=0;i<workers;i++){for(let y=0;y<canvasHeight/workers;y++){for(let x=0;x<canvasWidth;x++){let pixelPos=(x+y*canvasWidth)*4;mdSegment[i][pixelPos+3]=255;}}
...
eventTime=performance.now();needRedraw=1;startRender(1,1);}
...
logText.innerHTML="<i>Working...</i>";itersInput.value=iterations;juliaItersText.textContent=juliaIterations;zoomText.textContent=Math.floor(zoom);mc.style.borderColor="black";mc.style.outline="5px solid gray";if(lneedRecompute){rotating=0;cycleText.value="Cycle";}
...
function drawMandel()
{const iter_max=iterations;const lcanvasWidth=canvasWidth;const lcanvasHeight=canvasHeight;const lscreenX=screenX;const lscreenY=screenY;const lzoom=zoom;if((rotating)&&(performance.now()<rotationFrameStart+50)){requestAnimationFrame(drawMandel);return 1;}
...
if((drawJulia)&&(juliaIterations!=juliaIterSlider.value)){juliaIterations=Math.floor(juliaIterSlider.value);juliaItersText.textContent=juliaIterations;updateCoords(canvasWidth/2,canvasHeight/2,"centre");}
for(i=0;i<workers;i++)
if((blockSize[i]>1)&&(performance.now()>zoomTime+500))
needRedraw=1;if(needRedraw){for(i=0;i<workers;i++){startLine=chunkHeight*i;if(!computeWorker[i]){computeWorker[i]=new Worker("mandel-compute.js");computeWorker[i].onmessage=onComputeEnded;}
if((needToRun[i]==1)&&(!eventOccurred)){if(renderWorkerRunning[i]){continue;}
